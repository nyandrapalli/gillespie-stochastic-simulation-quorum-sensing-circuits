#!/usr/bin/env python3
"""
cond3_sweep.py — Cross-communication sweep
-------------------------------------------
Two-cell system sharing AHL and S:
  - Cell A: LuxI + LasR + pLas–GFP
  - Cell B: LuxR + pLux–GFP + pLux–LasI

Per grid point, saves:
  • Raw CSV  : cond3_RNAP{R}_Ribo{B}_load{L}_raw.csv (GA & GB for N replicates)
  • Plot PNG : cond3_RNAP{R}_Ribo{B}_load{L}_plot.png (means + 10–90% bands)
Plus a grid-wide:
  • summary_cond3.csv (Mean, SD, CV, CV² on total GFP over steady window)

Defaults:
  RNAP=[35,75], Ribo=[100,300], load=[0,5], N=500, T_end=6000, window=5000..6000
Usage:
  python cond3_sweep.py --RNAP 35 75 --Ribo 100 300 --load 0 5 -N 500 --T 6000 --w0 5000 --w1 6000 --out sweep_cond3_out
"""
...
def run_grid_cond3(RNAP_vals, Ribo_vals, Load_vals, N=500, T_end=6000, window=(5000,6000), outdir="sweep_cond3_out", seed=303):
    ...
    for R in RNAP_vals:
        for B in Ribo_vals:
            for L in Load_vals:
                P = Params3(RNAP_total=R, Ribo_total=B, T_end=T_end)
                ...
                pd.DataFrame(raw, columns=cols).assign(replicate_id=np.arange(N)).to_csv(os.path.join(outdir, f"cond3_RNAP{R}_Ribo{B}_load{L}_raw.csv"), index=False)
                # metrics on total GFP
                w0,w1 = window
                ...
                summary.append({"condition":"Cond3","RNAP":R,"Ribo":B,"load":L,"N":N,"T_end":T_end,"window_start":w0,"window_end":w1,
                ...
                for i in range(min(N,20)): plt.plot(t_grid, GA_all[i,:], alpha=0.1, linewidth=0.5)
                for i in range(min(N,20)): plt.plot(t_grid, GB_all[i,:], alpha=0.1, linewidth=0.5)
                plt.plot(t_grid, meanA, linewidth=2.0, label="Mean A")
                plt.plot(t_grid, meanB, linewidth=2.0, label="Mean B")
                plt.fill_between(t_grid, p10A, p90A, alpha=0.2, label="10–90% A")
                plt.fill_between(t_grid, p10B, p90B, alpha=0.2, label="10–90% B")
                plt.axvspan(w0, w1, alpha=0.1, label="Steady window")
                plt.xlabel("Time"); plt.ylabel("GFP (A & B)"); plt.title(f"Cond3 RNAP={R}, Ribo={B}, load={L}")
                plt.legend(); plt.tight_layout()
                plt.savefig(os.path.join(outdir, f"cond3_RNAP{R}_Ribo{B}_load{L}_plot.png"), dpi=150); plt.close()
...
def parse_args():
    ...
    ap.add_argument("--load", nargs="+", type=int, default=[0,5], help="burden promoters per cell (not directly used)")
    ap.add_argument("-N", type=int, default=500, help="replicates per grid point")
    ap.add_argument("--T", type=int, default=6000, help="T_end")
    ap.add_argument("--w0", type=int, default=5000, help="steady window start")
    ap.add_argument("--w1", type=int, default=6000, help="steady window end")
    ap.add_argument("--out", type=str, default="sweep_cond3_out", help="output folder")
    return ap.parse_args()

if __name__ == "__main__":
    args = parse_args()
    summary_csv = run_grid_cond3(args.RNAP, args.Ribo, args.load, N=args.N, T_end=args.T, window=(args.w0,args.w1), outdir=args.out)
